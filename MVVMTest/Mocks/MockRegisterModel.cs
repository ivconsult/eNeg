...
namespace citPOINT.eNeg.MVVM.UnitTest
{
    ...
    public class MockRegisterModel : IUserRegisterModel
    {
        ...
        public void CheckIsEmailExist(User CurrentUser)
        {
            ...
            {
                if (_users.Where(s => s.EmailAddress == CurrentUser.EmailAddress &&
                    s.UserID == CurrentUser.UserID &&
                    s.Disabled == false)
                    .FirstOrDefault() == null)
                    GetCheckIsEmailExistComplete(this, new eNegEntityResultArgs<User>(new List<User>()));
                else
                    GetCheckIsEmailExistComplete(this, new eNegEntityResultArgs<User>(new List<User>() { _users[0] }));
            }
        }
        ...
        public void UpdateReset(Guid? UserID)
        {
            ...
            {
                GetResetRequestComplete(this, new eNegEntityResultArgs<User>(_users.Where(s => s.UserID == UserID)));
            }
        }
        ...
        public void CheckUserRequestReset(string OperationString)
        {
            if (GetCheckUserRequestResetComplete != null)
            {
                ...
                {
                    DeleteUserOperationByUserIDComplete(this, new eNegEntityResultArgs<User>(new List<User>()));
                }
                else
                {
        ...
        {
            if (DeleteUserOperationByUserIDComplete != null)
            {
                var user = _users.Where(s => s.UserID == UserID).FirstOrDefault();
                if (user == null)
                {
                    DeleteUserOperationByUserIDComplete(this, new eNegEntityResultArgs<User>(new List<User>()));
                }
                ...
                {
                    DeleteUserOperationByUserIDComplete(this, new eNegEntityResultArgs<User>(new List<User>() { user }));
                }
            }
        }
        ...
        {
            if (GetAllApplicationComplete != null)
            {
                GetAllApplicationComplete(this, new eNegEntityResultArgs<Application>(Applications));
            }
        }
